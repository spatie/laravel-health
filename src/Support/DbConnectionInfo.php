...
class DbConnectionInfo
{
	...
	{
		return $connection->scalar('SELECT size_mb from (SELECT table_schema "name", ROUND(SUM(data_length + index_length) / 1024 / 1024, 1) "size_mb" FROM information_schema.tables GROUP BY table_schema) alias_one where name = ?', [$connection->getDatabaseName()]);
	}
